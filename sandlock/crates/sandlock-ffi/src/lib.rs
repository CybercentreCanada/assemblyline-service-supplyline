...
pub unsafe extern "C" fn sandlock_spawn(
    ...
    // Helper to set the Python error and return null
    let raise_error = |msg: &str| {
        Python::with_gil(|py| {
            PyValueError::new_err(msg.to_string()).restore(py);
        });
        ptr::null_mut()
    };

    if policy.is_null() { return raise_error("Policy pointer is null"); }
    if argv.is_null() { return raise_error("Argv pointer is null"); }

    let policy = &(*policy)._private;

    let name = match optional_name(name) {
        Ok(name) => name,
        Err(e) => return raise_error(&format!("Invalid name: {}", e)),
    };
    ...
    let rt = match tokio::runtime::Runtime::new() {
        Ok(rt) => rt,
        Err(e) => return raise_error(&format!("Failed to create Tokio runtime: {}", e)),
    };

    let mut sb = match Sandbox::new(policy, name.as_deref()) {
        Ok(sb) => sb,
        Err(e) => return raise_error(&format!("Sandbox initialization failed: {}", e)),
    };

    if let Err(e) = rt.block_on(sb.spawn_captured(&arg_refs)) {
        return raise_error(&format!("Spawn failed: {}", e));
    }
